...
public abstract class ASNDataReader implements ASNDataDecoder
{
	...
	protected byte readByte() throws ASNException
	{
		...
		try
		{
			i = buffIn.read();
			if (i == -1)
				throw new ASNEOFException(ASNErrorCodes.CODEC_DATA_CORRUPTION,
							new Object[] { "Data read error to meet the ECL requirement." });
		}
		catch (IOException e)
		{
	...
	public Boolean slide() throws Exception
	{
		...
		System.out.println("POSITION-> " + count + "\t :TAG-> " + tag.value()
					+ "\t: LENGTH-> " + length + "\t: VALUE-> "
	 ...
	 * @throws Exception
	 */
	public Boolean shift(FileWriter f) throws Exception
	{
		tag = readTag();
		length = readLength();
		content = null;
		...
		if (count > 913947)
		{
			System.out.println("ASNDataReader.shift() HERE I AM");
		}
